    ...
    extraReducers(builder) {
      builder.addCase(queryThunk.pending, (draft, {
        ...
        for (const [key, entry] of Object.entries(queries)) {
          if ((entry == null ? void 0 : entry.status) === "fulfilled" || (entry == null ? void 0 : entry.status) === "rejected") {
            draft[key] = entry;
          }
        }
      });
    ...
    extraReducers(builder) {
      builder.addCase(mutationThunk.pending, (draft, {
        ...
        for (const [key, entry] of Object.entries(mutations)) {
          if (((entry == null ? void 0 : entry.status) === "fulfilled" || (entry == null ? void 0 : entry.status) === "rejected") && key !== (entry == null ? void 0 : entry.requestId)) {
            draft[key] = entry;
          }
        }
      });
...
function buildSelectors({
  ...
  function selectCachedArgsForQuery(state, queryName) {
    return Object.values(state[reducerPath].queries).filter((entry) => (entry == null ? void 0 : entry.endpointName) === queryName && entry.status !== "uninitialized").map((entry) => entry.originalArgs);
  }
}
...
function buildHooks({
  ...
  function usePrefetch(endpointName, defaultOptions) {
    ...
    return (0, import_react.useCallback)((arg, options) => dispatch(api.util.prefetch(endpointName, arg, __spreadValues2(__spreadValues2({}, stableDefaultOptions), options))), [endpointName, dispatch, stableDefaultOptions]);
  }
  function buildQueryHooks(name) {
    ...
    return {
      ...
      useLazyQuery(options) {
        ...
        const queryStateResults = useQueryState(arg, __spreadProps2(__spreadValues2({}, options), {
      ...
      useQuery(arg, options) {
        ...
        const queryStateResults = useQueryState(arg, __spreadValues2({
        ...
        return (0, import_react.useMemo)(() => __spreadValues2(__spreadValues2({}, queryStateResults), querySubscriptionResults), [queryStateResults, querySubscriptionResults]);
      }
    };
  }
  function buildMutationHook(name) {
    return ({
      ...
      const finalState = (0, import_react.useMemo)(() => __spreadProps2(__spreadValues2({}, currentState), {
        originalArgs,
        reset
